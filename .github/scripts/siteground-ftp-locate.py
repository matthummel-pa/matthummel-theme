...
def child_web_roots(ftp: ftplib.FTP, path: str) -> list[str]:
    ...
    for name in nlst(ftp, path):
        ...
        if base in {"public_html", "www", "httpdocs", "wwwroot"} or "matthummel" in base:
            out.append(name if "/" in name or name.startswith("/") else join_ftp(path, base))
    return out


def siteground_domain_roots(ftp: ftplib.FTP) -> list[str]:
    """SiteGround often stores sites under /www/<domain>/public_html, not only /public_html."""
    out: list[str] = []
    for base in ("/", join_ftp("/", "www"), join_ftp("/", "domains")):
        names = nlst(ftp, base)
        if base == "/":
            log("FTP / names: " + ", ".join(sorted({folder_name(n) for n in names}))[:300])
        for name in names:
            folder = folder_name(name)
            low = folder.lower()
            if low in {".", ".."}:
                continue
            if "matthummel" in low or low in {"public_html", "www", "httpdocs"}:
                out.append(name)
                out.append(join_ftp(name, "public_html"))
                out.append(join_ftp(name, "www"))
                out.append(join_ftp(name, "wp-content/themes"))
...
def find_theme_styles(ftp: ftplib.FTP, roots: Iterable[str]) -> list[tuple[str, str]]:
    ...
    def consider(style_path: str) -> None:
        ...
        if kind:
            log(f"Found {kind} theme folder {short_path(style_path.rsplit('/', 1)[0])}")
...
def probe_php_files(ftp: ftplib.FTP, theme_dir: str) -> str:
    web = theme_dir
    for _ in range(4):
        web = web.rstrip("/").rsplit("/", 1)[0] or "/"
        for name in (".php-version", "php-version", ".phpver"):
            raw = download_text(ftp, join_ftp(web, name), limit=80).strip()
            if raw and raw[0].isdigit():
                log(f"PHP version file at …/{short_path(web)}/{name}: {raw[:20]}")
                return raw.split()[0]
    return ""


def probe_php(ftp: ftplib.FTP, theme_dir: str) -> str:
    from_file = probe_php_files(ftp, theme_dir)
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "matthummel-theme-deploy/3"})
...
def prefer_hint_paths(ftp: ftplib.FTP) -> Iterable[str]:
    ...
    expanded.extend(siteground_domain_roots(ftp))
...
def live_score(style_path: str, kind: str) -> int:
    ...
    if folder == "matthummel":
        ...
        score += 2
    # The public site is still the June FSE block theme. A Sage copy under
    # public_html/ is often a previous dump, not what nginx serves.
    if "matthummel.com" in path:
        score += 40
    if "/www/" in path:
        score += 12
    if kind == "fse":
...
def main() -> int:
    ...
    for path, kind in hits:
        log(f"Hit {kind} {short_path(path)}")
    ...
    fse_hits = [(path, kind) for path, kind in live_hits if kind in {"fse", "other"}]
    if fse_hits:
        live_hits = fse_hits + [h for h in live_hits if h not in fse_hits]
    if not live_hits:
    ...
    log(f"Live theme folder kind={live_kind} path=…/{short_path(live_dir)}")
