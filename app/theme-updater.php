...
add_action('customize_register', function (\WP_Customize_Manager $wp): void {
    ...
    $wp->add_control('mh_openai_token', [
        ...
        'description' => __('Used when exporting to DEV.to or drafting Bluesky summaries. Or set MH_OPENAI_API_KEY / OPENAI_API_KEY. Without it, rule-based copy still runs.', 'sage'),
        'section' => 'mh_devto',
        'type' => 'password',
    ]);
    ...
    $wp->add_control('mh_bluesky_auto_share', [
        ...
        'description' => __('When you publish a post, share a summary + link on Bluesky (~20s after publish). Skips DEV.to imports. Paste a custom summary in the Bluesky box on the post editor, or use the OpenAI key from DEV.to settings.', 'sage'),
        'section' => 'mh_bluesky',
        'type' => 'checkbox',
    ]);
