...
| Projects CPT | Studio projects imported once (`mh_projects_cpt_seeded_v1`); narrative/custom fields seed (`mh_concept_pages_seeded_v1`, `mh_concept_fields_admin_v1`); marketplace catalog seed for Acreline + TOCflow (`mh_product_catalog_v1` … `mh_product_catalog_v3`); editable project fields in wp-admin; list columns for On site (toggle), Category, Place with filters + newest-first sort; public singles at `/projects/{slug}/` (legacy `/concept/` 301s); **Generate featured image** on the edit screen (featured image wins over Screenshot file meta for Work/project cards and appears at right in the product hero); product pages include repo-reviewed benefits, screenshots, architecture, handoff, compatibility, license, docs, and buyer FAQ; opt-in products sync to the correct WooCommerce theme/plugin category |
