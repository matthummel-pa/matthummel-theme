...
| Journal | Featured latest post, hero search, newest/oldest sort, Grid/List, topics, years, tags, most discussed, numbered pagination, RSS; unique Read more links; source posts in `docs/posts/` as Gutenberg block markup (not Custom HTML); single-post hero shows featured image beside title/meta; comparison grids styled in the block editor via `journal-blocks.css` | `index.blade.php`, `archive.blade.php`, `partials/content-single.blade.php`, `partials/write-*.blade.php`, `partials/read-more.blade.php`, `resources/js/writing-tools.js`, `resources/css/journal-blocks.css`, `resources/css/editor.css`, `docs/posts/`, `.github/scripts/import-docs-posts.py` |
