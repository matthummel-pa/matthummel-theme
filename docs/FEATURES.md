...
- Buyer update mail is Woo email id `mh_customer_download_update`. Trigger only when the zip version increases. First attach does not email. `_mh_notified_version` on the line item prevents duplicates.
