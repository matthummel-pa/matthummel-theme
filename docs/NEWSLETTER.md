...
The first step also picks a layout: **Standard**, **Welcome**, **Plain**, **Feature**, or **Blog post**. They do not share one stack. Standard is a muted intro, a large title, then the letter, with the image under the title when the issue has one. Welcome is shorter, centered, and has no image. Plain is a white page with a navy rule beside the intro and a text link instead of a button. Feature puts a navy stripe, then the image full width, then the title. Blog post is an announcement: image, category, title, excerpt, the headings under **In this note**, then **Read the post**. On a Blog post letter, **Sections** can leave out the image, excerpt, headings, categories, or button. Picking Welcome does not email the list. **Letter style** on that settings screen is the default shell: Card, Banner, or Paper. Masthead can sit left or center. Welcome stays centered either way. The button can be solid or outline. The wizard has the same controls, and the preview updates when you change them. A saved letter keeps its own look. **Newsletter rules** can skip a post that was already sent and limit the Blog post picker to chosen categories. The postal address and the unsubscribe link stay on every letter. The dashboard shows the current letter in a small email preview. In the wizard, **Preview** is an email frame: from, subject, and the preview text. That line starts as the reusable intro. Desktop is about 600 pixels wide and Mobile is about 375. It updates as you change the layout, subject, note, or letter style. It does not send mail. **Advanced** is the other editor on that same screen. It lists intro, heading, body, an optional button, and sign-off. Empty intro and sign-off keep the reusable lines. Simple stays the usual path.
