...
3. In the block editor, open the **Code editor** (three dots → Code editor) and paste the `*.html` body.
4. Publish. Suggested live URL after permalinks: `/what-actually-gets-faster-with-ai/` (or `/web-development/what-actually-gets-faster-with-ai/` if the site uses `/%category%/%postname%/`).
...
Requires WP-CLI against `~/wp-site`. Skips a slug that already exists. Does not delete posts.
