...
function layouts(): array
{
    return [
        ...
        'feature' => [
            ...
            'summary' => __('A white card. The image sits inset, and the title sits directly under it.', 'matthummel-newsletter'),
        ],
        'post' => [
            'label' => __('Blog post', 'matthummel-newsletter'),
            'summary' => __('A white card: inset image, category chip, title, excerpt, headings, and Read the post.', 'matthummel-newsletter'),
        ],
    ];
}
...
function post_copy_row(string $html): string
{
    ...
    return '<table role="presentation" class="mhn-post-copy" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td style="padding:0;">'.$html.'</td></tr></table>';
}
...
function blog_post_categories_html(array $names): string
{
    ...
    return '<p class="mhn-post-cats" style="margin:0 0 14px;text-align:left;">'
        .'<span class="mhn-chip" style="display:inline-block;padding:4px 10px;background-color:#dceaf8;border-radius:4px;font-family:'.email_font_stack().';font-size:13px;line-height:1.4;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#0d2e57;">'.$line.'</span>'
        .'</p>';
}
...
function apply_layout(string $layoutId, string $body): string
{
    ...
    if ($layoutId === 'welcome') {
        ...
        $points = letter_is_detailed() ? letter_points_html(settings()['welcome_points']) : '';

        return strip_layout_images($html.$points);
    }
...
function arrange_layout_variant(string $layoutId, string $intro, string $image, string $heading, string $introHtml, string $body, string $signoff): string
{
    ...
    if ($layoutId === 'welcome') {
        $points = $detailed ? letter_points_html(settings()['welcome_points']) : '';
...
function letter_callout_html(string $label, string $text): string
{
    ...
    return '<table role="presentation" class="mhn-callout" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:4px 0 18px;"><tr>'
        ...
        .'<p style="margin:0 0 6px;font-size:12px;line-height:1.4;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#50575e;">'.esc_html($label).'</p>'
...
function layout_eyebrow_html(string $text, string $layoutId): string
{
    ...
    if ($layoutId === 'plain') {
        return '<table role="presentation" class="mhn-eyebrow-block" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 28px;"><tr>'
            .'<td class="mhn-eyebrow" style="border-left:3px solid #0d2e57;padding:2px 0 2px 16px;font-family:'.$font.';font-size:18px;line-height:1.45;font-weight:600;color:#50575e;text-align:left;">'.$inner.'</td>'
            .'</tr></table>';
    }
    if ($layoutId === 'welcome') {
...
function sample_issue_body(): string
{
    ...
    return $greeting.sample_feature_image().$heading.$excerpt.$follow.$close.$button;
}
...
function sample_feature_image(): string
{
    ...
    return '<img class="mhn-img" src="'.$safe.'" alt="'.esc_attr(__('A wide workshop still', 'matthummel-newsletter')).'" width="600" height="220" style="display:block;width:100%;max-width:600px;height:auto;border:0;margin:0;background-color:#eef3f9;">';
}
...
function render_email_emulator(array $view, bool $live, bool $compact): void
{
    ...
    echo '<button type="button" data-mhn-format="plain" aria-pressed="false">'.esc_html__('Plain email', 'matthummel-newsletter').'</button>';
    echo '</div>';
