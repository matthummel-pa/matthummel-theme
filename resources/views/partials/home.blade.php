@php
  ...
  $doCards = \App\mh_home_do_cards();
  $serviceCards = \App\mh_home_service_cards();
        ...
        <div class="h-hero__actions">
          @if ($hero['show_primary'] && $hero['cta_primary'] !== '')
            <a class="btn h-hero__cta" href="{{ esc_url($hero['cta_primary_url']) }}">
              {!! \App\mh_svg_icon('briefcase', 17) !!}
              {{ $hero['cta_primary'] }}
            </a>
          @endif
          @if ($hero['show_secondary'] && $hero['cta_secondary'] !== '')
            <a class="btn btn-outline h-hero__cta-secondary" href="{{ esc_url($hero['cta_secondary_url']) }}">
  ...
  <div class="container wide">
    <div class="h-journal__head">
      <div class="h-journal__head-copy">
        <p class="h-section-label">{{ __('02 · Journal', 'sage') }}</p>
...
<section class="h-skills h-band" id="do" aria-labelledby="h-do-heading">
  <div class="container wide">
    <div class="h-skills__head">
      <div>
        <p class="h-section-label">{{ __('03 · Practice', 'sage') }}</p>
        <h2 id="h-do-heading" class="h-section__title">
          {{ \App\field('home_build_h2', __('What I do.', 'sage')) }}
        </h2>
      </div>
      <p class="h-skills__note">
        {{ \App\field('home_build_note', __('For shops and agencies. Stack notes are on About and Code.', 'sage')) }}
      </p>
    </div>
    <ul class="h-do-list">
      @foreach ($doCards as $card)
        <li class="h-do-item">
          <span class="h-do-item__icon" aria-hidden="true">{!! \App\mh_svg_icon($card['icon'], 22) !!}</span>
          <div>
            <h3 class="h-do-item__title">{{ $card['title'] }}</h3>
            <p class="h-do-item__text">{{ $card['text'] }}</p>
          </div>
        </li>
      @endforeach
    </ul>
...
<section class="h-services h-band h-band--tint" id="services" aria-labelledby="h-svc-heading">
  <div class="container wide">
    <div class="h-skills__head">
      <div>
        <p class="h-section-label">{{ __('04 · Services', 'sage') }}</p>
        <h2 id="h-svc-heading" class="h-section__title">
          {{ \App\field('home_svc_h2', __('How I can help.', 'sage')) }}
        </h2>
        <p class="h-work-intro">
          {{ \App\field('home_svc_intro', __('Scoped WordPress and web work. Hire me for a build, overflow, or a full-time role.', 'sage')) }}
        </p>
      </div>
      <a class="h-text-arrow" href="{{ home_url('/hire/') }}">{{ __('Hire page', 'sage') }} →</a>
    </div>
    <div class="h-svc-grid">
      @foreach ($serviceCards as $card)
        <article class="h-svc-box">
          <span class="h-svc-box__icon" aria-hidden="true">{!! \App\mh_svg_icon($card['icon'], 28) !!}</span>
          <h3 class="h-svc-box__title">{{ $card['title'] }}</h3>
          <p class="h-svc-box__text">{{ $card['text'] }}</p>
        </article>
      @endforeach
    </div>
  </div>
