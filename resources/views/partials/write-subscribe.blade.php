...
<section class="write-subscribe" aria-labelledby="write-sub-h">
  <div>
    <h2 id="write-sub-h" class="display-title is-section">{{ \App\field('write_subscribe_h2', __('Subscribe in a reader', 'sage'), $writeId) }}</h2>
    <p class="sec-intro">{{ \App\field('write_subscribe_lede', __('No newsletter form. Copy the RSS URL into Feedly, NetNewsWire, or the reader you already use.', 'sage'), $writeId) }}</p>
  </div>
