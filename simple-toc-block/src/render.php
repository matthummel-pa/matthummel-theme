...
$all      = simple_toc_get_all_headings( $post_id );
...
$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => 'simple-toc' ) );
$list_tag           = ! empty( $attributes['ordered'] ) ? 'ol' : 'ul';
$title              = isset( $attributes['title'] ) ? $attributes['title'] : '';
$label              = '' !== $title ? $title : __( 'Table of Contents', 'simple-toc-block' );
?>
...
<nav <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> aria-label="<?php echo esc_attr( $label ); ?>">
	<?php if ( '' !== $title ) : ?>
		<p class="simple-toc__title"><?php echo esc_html( $title ); ?></p>
	<?php endif; ?>
	<?php
	// Markup is assembled with escaped values inside simple_toc_render_list().
	echo simple_toc_render_list( $filtered, $list_tag ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	?>
</nav>
